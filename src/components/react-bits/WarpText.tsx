...
// Adapted from React Bits' WarpText (https://reactbits.dev) -- ported to
// TypeScript and integrated as a purely decorative background flourish
// (see usage in Contact.tsx). Behavior is unchanged from the original;
// only typing and a couple of lint-driven refactors differ.
...
export interface WarpTextProps {
  ...
  /** When true, the root renders `aria-hidden` instead of `role="img"
   * aria-label={text}` -- for use as a purely decorative layer where a
   * sibling element (a real heading, a visually-hidden span, etc.) already
   * owns the accessible name, so the two don't announce redundantly. */
  decorative?: boolean;
}
